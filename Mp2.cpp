...
int main() {
	...
	while (isWinningScore(userTotalScore, cpuTotalScore) == false) {
		...
		while (turnDone == false) {
			...
			//checks if the user wants to roll or stop
			if (input == 'r' || input == 'R') {
				...
				//checks if the user rolled two 1's
				...
				//adds up the dice values and puts it in the current turn value
				else {
		...
		while (cpuCurrentTurn < 20 && isWinningScore(userTotalScore, cpuTotalScore) == false) {
			...
			//tells the user that its still the computers turn
			...
			//checks if the computer rolled two 1's and if so they lose the current turn
			if (isTotalScoreLost(die1value, die2value)) {
				cpuTotalScore = 0;
				cpuCurrentTurn = 0;
			...
			//checks if the computers turn is lost due to rolling a 1
			else if (isTurnScoreLost(die1value, die2value)) {
				cpuCurrentTurn = 0;
			...
			//when the computer reaches 20 or more on a turn it will stop the turn
...
//function used for rolling the pair of dice returns a single dice value 1-6
int rollDie(void) {
	int dieRollValue = 0;
...
char getUserInput(void) {
	...
	//checks that the input the user submitted is valid
...
bool isWinningScore(int userTotalScore, int cpuTotalScore) {
	if (isWinningScore(cpuTotalScore) || isWinningScore(userTotalScore)) {
		return true;
	}
	else {
		return false;
...
//checks if one of the scores is greater than or equal to 100
bool isWinningScore(int score) {
	if (score >= 100) {
		return true;
	}	
